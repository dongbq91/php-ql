...
if(isset($_GET['id'])) {
    $id=$_GET['id'];
    $sql1='SELECT * FROM internet WHERE MaGoiInternet="'.$id.'" ';
    $goicuoc= executeSingleResult($sql1);
    if($goicuoc != null){
        $tengoicuoc=$goicuoc['TenGoiInternet'];
        $phicuoc=$goicuoc['PhiGoiInternet'];
    }
}
if(!empty($_POST)){
    ...
    $macuoc=$_POST['macuoc'];
    $sql='UPDATE internet SET TenGoiInternet="'.$tengoicuoc.'", PhiGoiInternet="'.$phicuoc.'" WHERE MaGoiInternet="'.$macuoc.'"';

    if($goicuoc == null ){
        execute($sql);
        echo '<script language="javascript">';
        echo 'location.href="Internet_QuanLi.php"';
        echo '</script>';
        die();
    }else{
        echo '<script language="javascript">';
                    ...
                    <div class="card-body ">
                        <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'])?>"  method="post" onsubmit="">
                            <table class="table table-borderless">
                                ...
                                <tr>
                                    <td>
                                        <label for="uname" >Phí gói Internet (VND)</label>
