...
if(!empty($_POST)){
    ...
    $mainternet=$_POST['mainternet'];
    $time =$_POST['time'];
    $queryinternet="select * from internet where MaGoiInternet='".$mainternet."' ";
//    echo '<script language="javascript">';
//    echo 'alert("'.$mainternet.'")';
//    echo '</script>';
    $list=mysqli_query($conn,$queryinternet);
    while($row=mysqli_fetch_array($list)){
        $phigoi=$row['PhiGoiInternet'];
    }
    if(empty($phigoi)){
        $phigoi=0;
    }

    $sql='INSERT INTO chitietinternet (MaGoiInternet,PhiInternet,ThoiGian,SoThueBao) VALUES("'.$mainternet.'",'.$phigoi.',"'.$time.'","'.$sothuebao.'")';
    $sqlCheck="SELECT * FROM HoaDon where SoThueBao='".$sothuebao."'";
    $check=  mysqli_num_rows(mysqli_query($conn,$sqlCheck));
    if($check !=0){
        if(mysqli_query($conn,$sql)){
            $sql2='select * from chitietinternet where SoThueBao = "'.$sothuebao.'"';
            ...
            $sql1='UPDATE HoaDon SET TienSMS='.$tienSMS.',TienCuocGoi='.$tienCGHD.',TienInternet='.$tieninter.',VAT='.$vat.',TongTien='.$tongtien.' WHERE SoThueBao="'.$sothuebao.'" ';
...
<div class="container" style="max-width: 50%;">
    <div class="col-lg-12">
        <div class="row mb-4 mt-4">
            <div class="col-md-12">
                <h2>Thêm mới giao dịch Internet</h2>
            </div>
        </div>
        <div class="row">
            <div class="col-md-12">
                    ...
                    <div class="card-body ">
                        <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'])?>" method="post" onsubmit="validate()">
                            <table class="table table-borderless">
                                ...
                                <tr>
                                    <td> <label for="uname" >Tên gói internet (Sử dụng)</label>
                                        <select class="form-control" name="mainternet" id="mainternet">
                                            <option>---Chọn cước gọi sử dụng---</option>
                                            <?php
                                            $sqlTB='select * from internet';
                                            $listTB= executeResult($sqlTB);
                                            foreach ($listTB as $item){
                                                echo '<option value="'.$item['MaGoiInternet'].'">'.$item['TenGoiInternet'].'</option>';
                                            }
                                            ?>
                                        </select>
                                    </td>
