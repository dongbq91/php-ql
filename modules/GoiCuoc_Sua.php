...
if(isset($_GET['id'])) {
    $id=$_GET['id'];
    $sql1='SELECT * FROM cuocgoi WHERE MaCuocGoi="'.$id.'" ';
    $goicuoc= executeSingleResult($sql1);
    if($goicuoc != null){
        $tengoicuoc=$goicuoc['TenCuocGoi'];
        $phicuoc=$goicuoc['PhiCuocGoi'];
    }
}
if(!empty($_POST)){
    ...
    $macuoc=$_POST['macuoc'];
    $sql='UPDATE cuocgoi SET TenCuocGoi="'.$tengoicuoc.'", PhiCuocGoi="'.$phicuoc.'" WHERE MaCuocGoi="'.$macuoc.'"';

    if($goicuoc == null ){
        execute($sql);
        echo '<script language="javascript">';
        echo 'location.href="GoiCuoc_QuanLi.php"';
        echo '</script>';
        die();
    }else{
        echo '<script language="javascript">';
                    ...
                    <div class="card-body ">
                        <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'])?>"  method="post" onsubmit="">
                            <table class="table table-borderless">
                                ...
                                <tr>
                                    <td>
                                        <label for="uname" >Phí cước (VND/phút)</label>
